...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 11) {
            executeCatchedSQL(database, ALTER_TABLE + FeedColumns.TABLE_NAME + ADD + FeedColumns.ICON_DRAWABLE + ' ' + FeedData.TYPE_TEXT);
        }
        if (oldVersion < 12) {
            executeCatchedSQL(database, ALTER_TABLE + FeedColumns.TABLE_NAME + ADD + FeedColumns.NOTIFY + ' ' + FeedData.TYPE_BOOLEAN);
        }
    }
