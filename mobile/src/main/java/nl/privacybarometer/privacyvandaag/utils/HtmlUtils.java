...
public class HtmlUtils {
    ...
    private static final Pattern REMOVE_BEFORE_H1_PATTERN = Pattern.compile("<p>(.+?)</h1>", Pattern.DOTALL | Pattern.CASE_INSENSITIVE );
    private static final Pattern REMOVE_H1_PATTERN = Pattern.compile("<h1>(.+?)</h1>", Pattern.CASE_INSENSITIVE);
    ...
    public static String improveHtmlContent(String content, String baseUri) {
        ...
        if (content != null) {
            ...
            // clean by JSoup. Remove all non-whitelisted tags. Return safe HTML
            // This removes also all the classes and id's within the html-tags
            content = Jsoup.clean(content, baseUri, JSOUP_WHITELIST);

           // Log.e(TAG, "De opgeschoonde inhoud ziet er nu zo uit: " + content);
            ...
            // remove all text before the <h1>. Sometimes there is a subtitel or date
            content = REMOVE_BEFORE_H1_PATTERN.matcher(content).replaceAll("");
            // remove <h1> from document, because title is given seperately in rss feed.
            // TODO: If former match is made, this match is superfluous
            content = REMOVE_H1_PATTERN.matcher(content).replaceAll("");
